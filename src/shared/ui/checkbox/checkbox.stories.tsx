...
export const Controlled: Story = {
  render: (args) => {
    ...
    return (
      <div className="flex items-center space-x-2">
        <Checkbox {...args} id="controlled" checked={checked} onCheckedChange={setChecked} />
        <label
          htmlFor="controlled"
          className="text-sm leading-none font-medium peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
        >
          {checked === true ? 'Đang chọn (Click để bỏ)' : 'Chưa chọn (Click để chọn)'}
        </label>
      </div>
    );
  },
...
export const ErrorState: Story = {
  ...
  render: (args) => (
    <div className="flex items-center space-x-2">
      <Checkbox {...args} />
      <label htmlFor={args.id} className="text-destructive text-sm leading-none font-medium">
