...
export const categoryApi = {
  ...
  getAll: async () => {
    const res = await apiClient.get<ApiResponse<CategoryResponse[]>>(BASE_URL);
  ...
  create: async (payload: CreateCategoryPayload) => {
    const res = await apiClient.post<ApiResponse<CategoryResponse>>(BASE_URL, payload);
    return res.data.result;
  },

  // PUT /api/categories/{categoryId}
  update: async (id: string, payload: UpdateCategoryPayload) => {
    const res = await apiClient.put<ApiResponse<CategoryResponse>>(`${BASE_URL}/${id}`, payload);
    return res.data.result;
  },
