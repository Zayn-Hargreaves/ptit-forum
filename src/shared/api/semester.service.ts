...
import { SemesterResponse, CreateSemesterPayload } from '@/entities/semesters/model/types';
...
export const semesterApi = {
    ...
    getAll: async () => {
        const res = await apiClient.get<ApiResponse<SemesterResponse[]>>(
            `${BASE_URL}/all`
        );
    ...
    create: async (payload: CreateSemesterPayload) => {
        const res = await apiClient.post<ApiResponse<SemesterResponse>>(
            BASE_URL,
            payload
        );
        return res.data.result;
