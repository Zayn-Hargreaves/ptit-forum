...
export const cpaService = {
  initializeCpaProfile: async (): Promise<CpaProfileResponse> => {
    const response = await apiClient.post<ApiResponse<CpaProfileResponse>>('/cpa-profiles');
  ...
  getCpaProfile: async (id: string): Promise<CpaProfileResponse> => {
    const response = await apiClient.get<ApiResponse<CpaProfileResponse>>(`/cpa-profiles/${id}`);
    return response.data.result;
  },
