...
export function CommentSection({ postId }: CommentSectionProps) {
  ...
  if (comments && comments.length > 0) {
    ...
    const testRoot = comments.find(
      (c) => c.content.includes('test') && !c.content.includes('test 2'),
    );
  ...
  return (
    <div className="bg-muted/5 flex flex-col">
      ...
      <div className="space-y-6 px-4 py-2">
        ...
        {isError && <div className="py-2 text-xs text-red-500">Không thể tải bình luận</div>}
              ...
              .map((rootComment) => {
                ...
                return (
                  <div key={rootComment.id} className="space-y-2">
                    ...
                    {/* Replies Container */}
                    {replies.length > 0 && (
                      <div className="border-muted ml-11 space-y-3 border-l-2 pl-3">
                        {replies.map((reply) => (
                          <CommentItem
                            key={reply.id}
                            comment={reply}
