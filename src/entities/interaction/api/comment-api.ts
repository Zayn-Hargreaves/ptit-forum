...
export const commentApi = {
  getRootComments: async (postId: string, page = 0) => {
    const { data } = await apiClient.get<ApiResponse<PageResponse<Comment>>>(`/comments/post/${postId}`, {
  ...
  getReplies: async (rootCommentId: string, page = 0) => {
    const { data } = await apiClient.get<ApiResponse<PageResponse<Comment>>>(`/comments/replies/${rootCommentId}`, {
  ...
  create: async (payload: CreateCommentPayload) => {
    ...
    const { data } = await apiClient.post<ApiResponse<Comment>>(`/comments/post/${payload.postId}`, body);
    return data.result;
  },
